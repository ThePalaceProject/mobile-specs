...
Additionally, _device identifiers_ allow for unambiguous ownership information
for bookmarks. A device can know which bookmarks it needs to sync to the server
based on the _device identifier_ alone. Current applications do not have this
important information and so redundantly synchronize everything (often leading
to bookmarks that will not "go away" because some other device keeps sending
them to the server after a device has deleted them).
...
If you change the semantics, you should change the name.
